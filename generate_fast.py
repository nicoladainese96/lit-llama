...
def main(
    ...
    *,
    ...
    # define a DataLoader that prefetches data from the file system
    class CheckpointLoader(DataLoader):
        def __iter__(self):
            return BackgroundGenerator(super().__iter__())

    # create a DataLoader that loads the checkpoint file in parallel
    checkpoint_loader = CheckpointLoader(
        [checkpoint_path],
        batch_size=None,
        num_workers=8, # not sure about this
        pin_memory=True,
    )

    # load the checkpoint from the DataLoader
    with torch.no_grad():
        checkpoint = next(iter(checkpoint_loader))
